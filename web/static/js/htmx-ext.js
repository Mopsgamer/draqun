...
const slTypes = 'sl-radio-group, sl-rating, sl-input'
...
function shouldInclude(elt) {
	...
	if (['SL-RATING', 'SL-INPUT'].includes(elt.tagName) && elt.getAttribute('name')) {
		return true
	}
			...
			evt.detail.elt.querySelectorAll(slTypes).forEach((elt) => {
				if (shouldInclude(elt)) {
					if (['SL-RATING', 'SL-INPUT'].includes(elt.tagName)) {
						evt.detail.parameters.set(elt.getAttribute('name'), elt.value)
