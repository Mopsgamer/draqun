...
func (r ControllerHttp) GroupCreate() error {
	id := "new-group-error"
	req := new(model_http.GroupCreate)
	...
	r.HTMXRedirect(controller.PathRedirectGroup(group.Id))
	return r.RenderSuccess(i18n.MessageSuccCreatedGroup, id)
}

func (r ControllerHttp) GroupChange() error {
	id := "group-change-error"
	req := new(model_http.GroupChange)
...
func (r ControllerHttp) GroupDelete() error {
	id := "group-delete-error"
	req := new(model_http.GroupDelete)
	if err := r.Ctx.Bind().URI(req); err != nil {
		return r.RenderInternalError(id)
	}
